 ...
 ******************************************************************************/
#include <aphysics/collision/distances/evd_value.hpp>
#include <math.h>
...
float evd_value(float a_0, float a_1, float a_2, float b_0, float b_1, float b_2, float c_0, float c_1, float c_2) {
   const float x0 = a_0 - b_0;
   const float x1 = a_1 - b_1;
   const float x2 = a_2 - b_2;
   const float x3 = a_1 - c_1;
   const float x4 = a_0 - c_0;
   const float x5 = a_2 - c_2;

   float evd_value_result;
   evd_value_result = sqrt(pow(x0*x3 - x1*x4, 2) + pow(x0*x5 - x2*x4, 2) + pow(x1*x5 - x2*x3, 2))/sqrt(pow(x0, 2) + pow(x1, 2) + pow(x2, 2));
   return evd_value_result;

}

void evd_grad(float a_0, float a_1, float a_2, float b_0, float b_1, float b_2, float c_0, float c_1, float c_2, float *out_6809069808720211477) {
   const float x0 = a_0 - b_0;
   const float x1 = a_1 - b_1;
   const float x2 = a_2 - b_2;
   const float x3 = pow(x0, 2) + pow(x1, 2) + pow(x2, 2);
   const float x4 = a_1 - c_1;
   const float x5 = a_0 - c_0;
   const float x6 = x0*x4 - x1*x5;
   const float x7 = a_2 - c_2;
   const float x8 = x0*x7 - x2*x5;
   const float x9 = x1*x7 - x2*x4;
   const float x10 = sqrt(pow(x6, 2) + pow(x8, 2) + pow(x9, 2));
   const float x11 = x10/pow(x3, 3.0/2.0);
   const float x12 = 2*b_1;
   const float x13 = -2*c_1;
   const float x14 = x12 + x13;
   const float x15 = (1.0/2.0)*x6;
   const float x16 = 2*b_2;
   const float x17 = -2*c_2;
   const float x18 = (1.0/2.0)*x16 + (1.0/2.0)*x17;
   const float x19 = 1/(x10*sqrt(x3));
   const float x20 = 2*b_0;
   const float x21 = -2*c_0;
   const float x22 = -x20 - x21;
   const float x23 = (1.0/2.0)*x8;
   const float x24 = (1.0/2.0)*x9;
   const float x25 = 2*a_1;
   const float x26 = x13 + x25;
   const float x27 = 2*a_2;
   const float x28 = -x17 - x27;
   const float x29 = 2*a_0;
   const float x30 = x21 + x29;
   const float x31 = -x12 + x25;
   const float x32 = -x16 + x27;
   const float x33 = x20 - x29;

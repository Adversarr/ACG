...
private:
  HyperElasticResult<Scalar, dim> ComputeGradientImpl() const noexcept {
    ...
    result.energy = .5 * mu_ * (inv.i2_ - math::constant<Scalar>(3)) - mu_ * log(inv.i3_)
                    + lambda_ * 0.5 * math::square(inv.i3_ - 1);
    result.grad
        = .5 * mu_ * inv.i2_grad_
          + (lambda_ * (inv.i3_ - math::constant<Scalar>(1)) - mu_ / inv.i3_) * inv.i3_grad_;
    return result;
  }
  Scalar ComputeEnergyImpl() const noexcept {
    auto inv = SmithInvariants{deformation_gradient_}.ComputeValue();
    auto energy = .5 * mu_ * (inv.i2_ - math::constant<Scalar>(3)) - mu_ * log(inv.i3_)
                  + lambda_ * 0.5 * math::square(inv.i3_ - 1);
    return energy;
  }

  HyperElasticResultWithHessian<Scalar, dim> ComputeHessianImpl() const noexcept {
    ...
    result.energy = .5 * mu_ * (ig.i2_ - math::constant<Scalar>(3)) - mu_ * log(ig.i3_)
...
private:
  ...
  inline HyperElasticResult<Scalar, dim> ComputeGradientImpl() const noexcept {
    ...
    result.energy = .5 * mu_ * (inv.i2_ - 3) - mu_ * (inv.i3_ - 1)
                    + lambda_ * 0.5 * math::square(inv.i3_ - 1);
    result.grad = .5 * mu_ * inv.i2_grad_ + (lambda_ * (inv.i3_ - 3) + mu_) * inv.i3_grad_;
    return result;
  }

  inline HyperElasticResultWithHessian<Scalar, dim> ComputeHessianImpl() const noexcept {
    HyperElasticResultWithHessian<Scalar, dim> result;
    auto inv = SmithInvariants{deformation_gradient_}.ComputeHessian();
    auto& ig = inv.inv_grad_result_;
    result.energy
    ...
    result.grad = di2 * ig.i2_grad_ + di3 * ig.i3_grad_;
    result.hessian = details::compose_final_hessian<Scalar, dim>(0, 0, lambda_, 0, di2, di3, inv);
    return result;
  }
  Scalar lambda_, mu_;
  Mat<Scalar, dim, dim> deformation_gradient_;

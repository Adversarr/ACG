...
template <typename T, typename E = Status> class Result final {
public:
  Result(const T& data) : value_(data), is_inited_(true) {}
  ...
  template <typename... Args> static Result<T, E> Emplace(Args&&... data) {
    return Result(T(data...));
  }

  const T& Value() const {
    ACG_CHECK(is_inited_, "Bad Access Error");
    return value_;
  }

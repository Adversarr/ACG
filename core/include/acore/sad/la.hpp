...
template <typename X, typename Y> struct Dot
  ...
  auto operator()(XIn&& in_x, YIn&& in_y) {
...
template <typename X> struct Dot<X, X>
  ...
  template <typename XIn> auto operator()(XIn&& in_x) {
...
template <typename X> struct Inverse : public Expr<GetRunTimeType_t<X>, X> {
  ...
  template <typename XIn> auto operator()(XIn&& in_x) { return in_x.inverse(); }

  template <typename XIn, typename G> using Grad_t
      = Neg<Mul<Mul<Inverse<X>, G>, Inverse<X>>>;
};
...
template <typename L, typename R> struct Concatenate
    : public Expr<acg::Mat<typename Trait<GetRunTimeType_t<L>>::Scalar,
                           Trait<GetRunTimeType_t<L>>::rows,
                           Trait<GetRunTimeType_t<L>>::cols
                               + Trait<GetRunTimeType_t<R>>::cols>,
                  L, R> {
  ...
  template <typename XIn, typename YIn>
  auto operator()(XIn&& in_x, YIn&& in_y) {
    acg::Mat<typename Trait<GetRunTimeType_t<L>>::Scalar,
             Trait<GetRunTimeType_t<L>>::rows,
             Trait<GetRunTimeType_t<L>>::cols
                 + Trait<GetRunTimeType_t<R>>::cols>
        ret;
  ...
  template <typename XIn, typename G> using Grad_t = std::conditional_t<
      std::is_same_v<L, R>, Concatenate<G, G>,
...
template <typename X, int rows, int cols> using Reshape
    = details::Reshape<X, rows, cols>;
template <typename X, typename Y> using Concatenate
    = details::Concatenate<X, Y>;

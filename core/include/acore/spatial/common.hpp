...
template <typename F, int dim> struct AABB<void, F, dim> {
  ...
  inline bool Intersect(const AABB<Tp, F, dim> &another, F lim) const {
    // HACK: Unroll the loop
    for (Index i = 0; i < dim; ++i) {
      if ((upper_bound(i) - another.lower_bound(i)) *
              (another.upper_bound(i) - lower_bound(i)) <
          -lim) {
        return false;
      }
    }
  ...
  template <typename Tp>
  inline bool Contain(const AABB<Tp, F, dim> &another) const {
...
struct AABB : public AABB<void, F, dim> {
  using bound_type = acg::Vec<F, dim>;
  T data;
  AABB(bound_type lb, bound_type ub, const T &d)
      : AABB<void, F, dim>(lb, ub), data(d) {}
  AABB(const AABB &) = default;
  AABB(AABB &&) = default;
};

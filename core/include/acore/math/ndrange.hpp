...
template <Index dim> constexpr auto get_zero_tuple() {
  if constexpr (dim > 0) {
    return std::tuple_cat(get_zero_tuple<dim - 1>(),
                          std::make_tuple<acg::Index>(0));
  } else {
...
template <int dim> struct NdRange {
  ...
  constexpr decltype(auto) end() const {
    return NdRangeIterator<dim>(
        details::get_zero_tuple<dim>(std::get<0>(dims_)), dims_);
  }
  const container_type dims_;
};

template <typename... Args>
NdRange(std::tuple<Args...> &&dims) -> NdRange<sizeof...(Args)>;

...
float eed_value(float a_0, float a_1, float a_2, float b_0, float b_1, float b_2, float c_0, float c_1, float c_2, float d_0, float d_1, float d_2) {
   ...
   float eed_value_result;
   eed_value_result = (-x4*(a_2 - c_2) + x7*(a_1 - c_1) - x8*(a_0 - c_0))/sqrt(pow(x4, 2) + pow(x7, 2) + pow(x8, 2));
   return eed_value_result;

}

void eed_grad(float a_0, float a_1, float a_2, float b_0, float b_1, float b_2, float c_0, float c_1, float c_2, float d_0, float d_1, float d_2, float *out_4484199130847538343) {
   const float x0 = a_1 - c_1;
   const float x1 = c_2 - d_2;
   const float x2 = a_2 - c_2;
   const float x3 = -x2;
   const float x4 = c_1 - d_1;
   const float x5 = a_2 - b_2;
   const float x6 = x4*x5;
   const float x7 = a_1 - b_1;
   const float x8 = x1*x7;
   const float x9 = a_0 - b_0;
   const float x10 = x4*x9;
   const float x11 = c_0 - d_0;
   const float x12 = x11*x7;
   const float x13 = x10 - x12;
   const float x14 = x1*x9;
   const float x15 = x11*x5;
   const float x16 = x14 - x15;
   const float x17 = -x6 + x8;
   const float x18 = pow(x13, 2) + pow(x16, 2) + pow(x17, 2);
   const float x19 = pow(x18, -1.0/2.0);
   const float x20 = 2*c_1 - 2*d_1;
   const float x21 = (1.0/2.0)*x13;
   const float x22 = 2*c_2 - 2*d_2;
   const float x23 = (1.0/2.0)*x22;
   const float x24 = a_0 - c_0;
   const float x25 = (x0*x16 - x13*x2 - x17*x24)/pow(x18, 3.0/2.0);
   const float x26 = -x24;
   const float x27 = -x11;
   const float x28 = 2*c_0 - 2*d_0;
   const float x29 = -x28;
   const float x30 = -x4;
   const float x31 = (1.0/2.0)*x16;
   const float x32 = -x20;
   const float x33 = (1.0/2.0)*x17;
   const float x34 = -x1;
   const float x35 = -x22;
   const float x36 = -x5;
   const float x37 = -x7;
   const float x38 = 2*a_1 - 2*b_1;
   const float x39 = -x38;
   const float x40 = 2*a_2 - 2*b_2;
   const float x41 = -x40;
   const float x42 = 2*a_0 - 2*b_0;
   const float x43 = -x9;
   const float x44 = -x42;
